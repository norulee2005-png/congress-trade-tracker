...
export async function getPoliticianTrades(politicianId: string, limit: number = 100) {
  return db
    .select({
      id: trades.id,
...
export async function getPoliticianTopStocks(politicianId: string, limit: number = 5) {
  return db
    .select({
      stockTicker: trades.stockTicker,
      stockName: trades.stockName,
